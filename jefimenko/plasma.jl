...
#function plasma_simulation(grid, test=False):
#
#    start = Time.time()
...
function boris_method(location_list,
                      E_field,
                      H_field,
                      B_field,
                      time,
    ...
    for n in range(len(grid.charges[time])):
        ...
        V_minus = (charges[time][n].velocity + charges[time][n].Q * E_field[n] /
        ...
        Velocity = V_plus + charges[time][n].Q *
                   E_field[n] / charges[time][n].mass *
                   grid.delta_t / 2
